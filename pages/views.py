...
def SalesView(request):
  ...
  if request.method == 'POST':
    ...
    if form_data.is_valid():
      subject = u'Заявка на покупку металлопроката: ' + form_data.cleaned_data['first_name'] + ' ' + form_data.cleaned_data['patronym'] + ' ' + form_data.cleaned_data['last_name']

      messageHTML = u'<html><head><meta charset="utf-8"></head><body>'
      message = u'Имя: ' + form_data.cleaned_data['first_name'] + '\n'
      messageHTML += u'<p><span style="font-weight:bold">Имя: </span>' + form_data.cleaned_data['first_name'] + u'</p>'
      message += u'Отчество: ' + form_data.cleaned_data['patronym'] + '\n'
      messageHTML += u'<p><span style="font-weight:bold">Отчество: </span>' + form_data.cleaned_data['patronym'] + u'</p>'
      message += u'Фамилия: ' + form_data.cleaned_data['last_name'] + '\n' + '\n'
      messageHTML += u'<p><span style="font-weight:bold">Фамилия: </span>' + form_data.cleaned_data['last_name'] + u'</p><br>'
      message += u'Способ доставки: ' + form_data.cleaned_data['delivery_type'] + '\n'
      messageHTML += u'<p><span style="font-weight:bold">Способ доставки: </span>' + form_data.cleaned_data['delivery_type'] + u'</p>'
      if form_data.cleaned_data['delivery_type'] == u'Железной дорогой':
        message += u'Станция: ' + form_data.cleaned_data['station'] + '\n' + '\n'
        messageHTML += u'<p><span style="font-weight:bold">Станция: </span>' + form_data.cleaned_data['station'] + u'</p><br>'
      elif form_data.cleaned_data['delivery_type'] == u'Автотранспортом':
        message += u'Адрес: ' + form_data.cleaned_data['address'] + '\n' + '\n'
        messageHTML += u'<p><span style="font-weight:bold">Адрес: </span>' + form_data.cleaned_data['address'] + u'</p><br>'
      else:
        message += '\n'
        messageHTML += u'<br>'
      if form_data.cleaned_data['message']:
        message += u'Сообщение: ' + form_data.cleaned_data['message'] + '\n' + '\n'
        messageHTML += u'<p><span style="font-weight:bold">Сообщение: </span>' + form_data.cleaned_data['message'] + u'</p><br>'
  ...
  return render(request, 'pages/services_sale.html', context = {'titles': titles, 'categories': categories, 'form': form_data}, content_type = 'text/html')
...
def PurchasesView(request):
  ...
  if request.method == 'POST':
    ...
    if form_data.is_valid():
      ...
      messageHTML = u'<p><span style="font-weight:bold">Имя: </span>' + form_data.cleaned_data['first_name'] + '</p>'
      messageHTML += u'<p><span style="font-weight:bold">Отчество: </span>' + form_data.cleaned_data['patronym'] + '</p>'
      messageHTML += u'<p><span style="font-weight:bold">Фамилия: </span>' + form_data.cleaned_data['last_name'] + '</p><br>'

      messageHTML += u'<p><span style="font-weight:bold">Телефон: </span>' + form_data.cleaned_data['phone'] + '</p>'
      messageHTML += u'<p><span style="font-weight:bold">E-mail: </span>' + form_data.cleaned_data['email'] + '</p><br>'

      if form_data.cleaned_data['city']:
        messageHTML += u'<p><span style="font-weight:bold">Город: </span>' + form_data.cleaned_data['city'] + '</p><br>'

      if form_data.cleaned_data['amount']:
        messageHTML += u'<p><span style="font-weight:bold">Количество: </span>' + form_data.cleaned_data['amount'] + '</p><br>'
      ...
      if form_data.cleaned_data['deliverable']:
        messageHTML += u'<p><span style="font-weight:bold">Возможность доставки: </span>' + form_data.cleaned_data['deliverable'] + '</p>'
      if form_data.cleaned_data['delivery_type']:
        messageHTML += u'<p><span style="font-weight:bold">Способ доставки: </span>' + form_data.cleaned_data['delivery_type'] + '</p><br>'
      else:
        messageHTML += '<br>'

      if form_data.cleaned_data['message']:
        messageHTML += u'<p><span style="font-weight:bold">Сообщение: </span>' + form_data.cleaned_data['message'] + '</p>'
  ...
  return render(request, 'pages/services_purchase.html', context = {'price': prices, 'form': form_data}, content_type = 'text/html')
...
def VacanciesView(request):
  ...
  if request.method == 'POST':
    ...
    if form_data.is_valid():
      ...
      if form_data.cleaned_data['city']:
        messageHTML += u'<p><span style="font-weight:bold">Город: </span>' + form_data.cleaned_data['city'] + '</p>'

      if form_data.cleaned_data['street']:
        messageHTML += u'<p><span style="font-weight:bold">Улица: </span>' + form_data.cleaned_data['street'] + '</p><br>'
      else:
        messageHTML += '<br>'

      messageHTML += u'<p><span style="font-weight:bold">Вакансия: </span>' + form_data.cleaned_data['vacancie'] + '</p><br>'
...
def ContactsView(request):
  ...
  if request.method == 'POST':
    ...
    if form_data.is_valid():
      ...
      if form_data.cleaned_data['phone']:
        messageHTML += u'<p><span style="font-weight:bold">Телефон: </span>' + form_data.cleaned_data['phone'] + '</p>'
      messageHTML += u'<p><span style="font-weight:bold">Email: </span>' + form_data.cleaned_data['email'] + '</p>'
      if form_data.cleaned_data['message']:
        messageHTML += u'<p><span style="font-weight:bold">Сообщение: </span>' + form_data.cleaned_data['message'] + '</p>'
